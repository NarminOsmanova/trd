...
interface Props {
  ...
  onView: (item: CompanyItem) => void;
}

export default function CompaniesTable({ data, onEdit, onDelete, onToggleActive, onView }: Props) {
  return (
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Aylıq Büdcə Limiti</th>
              ...
              <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900">
                    {c.budgetLimit ? formatCurrency(c.budgetLimit) : '—'}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <button
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div className="flex items-center justify-end space-x-2">
                    <Button variant="ghost" size="sm" onClick={() => onView(c)}>
                      <Eye className="w-3 h-3 mr-1" /> Baxış
                    </Button>
